  ...
  struct PGen : public arch::ProblemGenerator<S, M> {
    ...
    void CustomPostStep(timestep_t step, simtime_t time, Domain<S, M>& domain) {
      ...
      for (auto& s : { 1u, 2u }) {
        ...
        auto  i1_prev      = species.i1_prev;
        auto  dx1_prev     = species.dx1_prev;
        ...
        Kokkos::parallel_for(
          ...
          Lambda(index_t p) {
            ...
            const auto x_Cd_prev = static_cast<real_t>(i1_prev(p)) +
                              static_cast<real_t>(dx1_prev(p));
            const auto x_Ph_prev = mesh.metric.template convert<1, Crd::Cd, Crd::XYZ>(
              x_Cd_prev);
            ...
            if (x_Ph < left_threshold) {
              ...
              // reflect particles that have just entered the hot reservoir back into the domain with a reversed velocity
              if (x_Ph_prev >= left_threshold) {
                ux1(p) = math::abs(v_Cd[0]);
              }
              ...
              // reflect particles that have just entered the cold reservoir back into the domain with a reversed velocity
              if (x_Ph_prev <= right_threshold) {
                ux1(p) = -math::abs(v_Cd[0]);
              }
            }
          });
