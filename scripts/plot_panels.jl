...
function read_field_data(info, t, x_l, dim=2)
    ...
    data = read_field(info, ["N_1",
                            "B1", "B2", "B3", 
                            "E1", "E2", "E3", "T00_1", "V1_1", "V2_1", "V3_1"], verbose=true; t)
    ...
    if dim == 2
        return data["X1"], data["X1"], data["X2"], data["N_1"], data["E1"], data["E2"], data["E3"], data["B1"], data["B2"], data["B3"], T, data["t"], data["step"]
    elseif dim == 3
...
function plot_shock_wide(sim_path, t, dim)
    ...
    @info "E"
    # electric field
    Colorbar(f[3:5, 2], limits=e_lim, colormap=:RdBu, label=L"E", tickalign=1)#, scale=Escale, ticks=[-0.1, -0.01, -0.001, 0, 0.001, 0.01, 0.1])
    ...
    ax = Axis(f[6, 1], aspect=((x_l[2]-x_l[1])/sum(abs.(y_l))), xlabel=L"x \:\: \left[ c/\omega_{pe} \right]", ylabel="E")#,
    ...
    # obliquity
    theta_lim = (0.05, 0.2)
    Colorbar(f[1, 4], limits=theta_lim, colormap=:magma, label=L"T", tickalign=1)
    ...
    ax = Axis(f[2, 3], aspect=((x_l[2]-x_l[1])/sum(abs.(y_l))), xlabel=L"x \:\: \left[ c/\omega_{pe} \right]", ylabel=L"T")
    xlims!(ax, x_l )
    ylims!(ax, theta_lim)
    ...
    Colorbar(f[3:5, 4], limits=b_lim, colormap=:PiYG, label=L"B - B_0", tickalign=1)
    ...
    ax = Axis(f[6, 3], aspect=((x_l[2]-x_l[1])/sum(abs.(y_l))), xlabel=L"x \:\: \left[ c/\omega_{pe} \right]", ylabel=L"B")
    xlims!(ax, x_l )
    ylims!(ax, b_lim)
...
sim_path = "/lus/flare/projects/CRsInCosmoSims/PIC/Paper/heat_conduction/runs/2D/komarov_new/_conduction/"
info = EntityData(sim_path)
